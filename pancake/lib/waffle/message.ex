defmodule Waffle.Message do
    ...
    @primary_key false
    embedded_schema do
      field(:operator, Waffle.Message.Types.Operator, null: false)
      field(:payload, :map)
    ...
    @type t :: %__MODULE__{
            operator: module(),
            payload: map(),
            reference: Pancake.Utils.UUID.t(),
            inbound_operator: String.t()
          }
  
    @spec changeset(%{String.t() => Waffle.json()}, Waffle.SocketHandler.state()) :: Changeset.t()
    @doc """
    ...
    @operators Waffle.Message.Manifest.actions()
    ...
    defp cast_payload(changeset, state) do
      ...
      |> case do
        ...
        inner_changeset = %{valid?: false} ->
          errors = Pancake.Utils.Errors.changeset_errors(inner_changeset)
