defmodule Waffle.SocketHandler do
  ...
  defp general_impl(message, state) do
    case message do
      {"game:update", data} when state.player != nil ->
        state = %{state | player: Map.get(data.players, state.player.id)}
        target = Map.get(data.players, state.player.target)

        ws_push(prepare_socket_msg(%{operator: "game:update", payload: %{started: data.started, players: map_size(data.players), turn: data.turn == state.player.id, boats: state.player.boats, shots: state.player.shots, target: Sushi.Schemas.Player.filterPlayer(target)}}), state)
      {"game:kick:" <> _, reason} when state.player != nil ->
        Hamburger.Game.removePlayer(state.player.id)
        state = %{state | player: nil}
        ws_push(prepare_socket_msg(%{operator: "game:kick", payload: %{reason: reason}}), state)
      _ -> ws_push(nil, state)
    end
  end
